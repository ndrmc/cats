...
IF EXISTS (SELECT * FROM [dbo].[ApplicationSetting] WHERE [SettingName] = 'LocalPurchaseReceiptPlanWorkflow')
BEGIN    
 select top 1 @ProcessTemplateID = SettingValue FROM [dbo].[ApplicationSetting] WHERE [SettingName] = 'LocalPurchaseReceiptPlanWorkflow'
 if(not exists(select * from StateTemplate where ParentProcessTemplateID = @ProcessTemplateID and Name = 'Deleted'))
   begin 
	INSERT INTO [dbo].[StateTemplate]
     ...
     VALUES
           ( @ProcessTemplateID,'Deleted',0,2,1)  select @edited = Scope_Identity()		   
   End
   else
      begin 
