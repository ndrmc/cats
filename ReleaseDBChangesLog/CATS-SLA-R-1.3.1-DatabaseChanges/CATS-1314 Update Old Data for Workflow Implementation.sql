...
SELECT GiftCertificateId FROM GiftCertificate
...
SET @status = ( SELECT [StatusID] FROM GiftCertificate WHERE GiftCertificateId  = @rowid );
IF (@status = 1) 
...
VALUES ( @ProcessTemplateID, @stateid, 'System: Data Migration', '2016-06-06', ' GiftCertificate business process created by data migrator', NULL, NULL) select @currentStateID = Scope_Identity();
...
INSERT INTO BusinessProcess ([ProcessTypeID]
           ...
           ,[DocumentType]
           ,[CurrentStateID])
VALUES ( @ProcessTemplateID, 0, 'GiftCertificateWorkflow', @currentStateID);
