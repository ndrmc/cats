...
IF  EXISTS (SELECT * FROM [dbo].[ProcessTemplate] WHERE [Name] = 'Regional Request') 
BEGIN
Select @ProcessTemplateID =  ProcessTemplateID  FROM [dbo].[ProcessTemplate]
where [Name] = 'Regional Request'
...
SELECT [RegionalRequestID] FROM [EarlyWarning].[RegionalRequest]
...
SET @status = ( SELECT [Status] FROM [EarlyWarning].[RegionalRequest] WHERE [RegionalRequestID]  = @rowid );
IF (@status = 1) 
...
ELSE IF (@status = 4)
SET @stateid = ( SELECT [StateTemplateID] FROM [StateTemplate] WHERE ParentProcessTemplateID = @ProcessTemplateID AND Name = 'FederalApproved' );
ELSE IF (@status = 5)
...
VALUES ( @ProcessTemplateID, @stateid, 'System: Data Migration', '2016-06-06', ' Regional Request business process created by data migrator', NULL, NULL) select @currentStateID = Scope_Identity();
...
INSERT INTO BusinessProcess ([ProcessTypeID]
           ...
           ,[DocumentType]
           ,[CurrentStateID])
VALUES ( @ProcessTemplateID, 0, 'RegionalRequest', @currentStateID);
...
UPDATE [EarlyWarning].[RegionalRequest]
SET BusinessProcessID=@businessprocessid
WHERE [RegionalRequestID] = @rowid;
