...
IF   EXISTS (
  SELECT * 
  FROM   sys.columns 
  WHERE  object_id = OBJECT_ID(N'[dbo].[GiftCertificate]') 
...
where [Tab].[Name] = 'GiftCertificate' and SysObjects.[Name] like 'DF__GiftCerti__Busin%'
order by [Tab].[Name]) 
EXEC('ALTER TABLE [dbo].[GiftCertificate] DROP CONSTRAINT ' + @constraintName)
