...
SELECT LocalPurchaseID FROM [dbo].[LocalPurchase]  
/* Check if the above statement failed to execute and rollback the transcation */
IF(@@error <> 0)
ROLLBACK /* Rollback of the transaction */
...
SET @status = ( SELECT [StatusID] FROM [dbo].[LocalPurchase] WHERE LocalPurchaseID = @rowid );
IF (@status = 1)
...
<ProcessTemplate Name="Local Purchase reciept plan" Description="Local Purchase Receipt Plan Processing" GraphicsData="NULL" PartitionId="0" />
